...
import { comparePassword, generateOtp, hashPassword, sendEmail, signToken, SUCCESS } from "../utils/helpers";
...
const verifyUserEmail = async (req: Request, res: Response, next: NextFunction): Promise<any> => {
    try {
        ...
        if (user.isEmailVerified && (type === 1 || type == "1")) {
            return SUCCESS(res, 200, "This email address has already been verified. You can log in to your account.",
                { user: userData(user) });
        }
...
export const updateProfile = async (req: Request, res: Response, next: NextFunction) => {
    try {
        ...
        return SUCCESS(res, 200, "Profile updated successfully", { user: userData });
    } catch (error) {
        next(error);
    }
};
...
const resetPassword = async (req: Request, res: Response, next: NextFunction): Promise<any> => {
    try {
        ...
        if (!user || !user.isEmailVerified) {
            throw new ErrorHandler(
                "User does not exist or email not verified hellohello",
                400
            );
        }
        if (!user.otpVerified) {
...
const forgetPassword = async (req: Request, res: Response, next: NextFunction): Promise<any> => {
    try {
        ...
        if (!user || !user.isEmailVerified) {
            throw new ErrorHandler(
                "User does not exist or email not verified",
                400
            )
        }
...
export const resendOtp = async (req: Request, res: Response, next: NextFunction): Promise<any> => {
  ...
  `,
            ...
            return SUCCESS(res, 200, "OTP resent successfully for email verification");
        }
        if (type === 2) {
            if (!user.isEmailVerified) {
                return next(new ErrorHandler("User does not exist or email not verified.", 400))
